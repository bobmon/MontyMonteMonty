...
class MontyHall:
    ...
    def playgame(self):
        ...
        if self.choice == self.winner:
            ...
            print('Error!', self.shown, self.doors[self.shown], file=sys.stderr)
    #----
    def showbad(self):
    ...
    def notchosen(self):
        ...
        if len(choices):
            print('OOPS! ', choices, file=sys.stderr)
    #----

    def displayResult(self):
        print("game: %2u"%self.nGames, end=': ')
        print("Choice: %u"%(self.choice), end='; ')
        print("Shown: %u is a %s"%(self.shown, self.doors[self.shown]), end='; ')
        print("Other: %u is a %s"%(self.other, self.doors[self.other]), end='; ')
        if self.choice == self.winner:
